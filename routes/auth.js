...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user.approved === 0) {
      return res
        .status(403)
        .json({ error: "Account not yet approved by admin" });
    }
    ...
    if (!user.two_fa_enabled) {
      ...
      if (rememberDevice) {
        ...
        await User.createTrustedDevice(
          user.id,
          deviceToken,
          req.ip,
...
router.post("/verify-2fa", async (req, res) => {
  ...
  try {
    ...
    if (
      !user ||
      String(user.temp_otp) !== otp ||
      !user.temp_otp_expiry ||
      user.temp_otp_expiry < new Date()
    ) {
      await User.incrementFailedLoginAttempts(email);
...
router.post("/verify-device", async (req, res) => {
  ...
  if (!jwtToken || !deviceToken) {
    return res.status(400).json({
      authenticated: false,
      error: "Missing jwtToken or deviceToken.",
    });
  }
